    ...
    with tab2:
        ...
        with col1:
            ...
            if fluo_file:
                ...
                st.image(image, caption="Fluorescein Image", use_column_width=True)
        ...
        if st.button("Generate Comprehensive Analysis", use_container_width=True):
            with st.spinner("Calculating..."):
                ...
                with col1:
                    ...
                    data = pd.DataFrame({
                        'Parameter': ['TBUT', 'TMH', 'Schirmer', 'LIPCOF', 'OSDI', 'DEQ-5'],
                        'Value': [f"{tbut}s", f"{tmh}mm", f"{schirmer}mm", f"{lipcof_total}/6", f"{osdi_score}/100", f"{deq5_score}/22"]
                    })
    ...
    with tab4:
        ...
        if 'analysis_results' in st.session_state:
            ...
            **Date:** {datetime.now().strftime('%Y-%m-%d %H:%M')}
            ...
            - **Assessment Score:** {st.session_state.analysis_results['score']}/12
            
            ### RECOMMENDATIONS
            1. Use preservative-free artificial tears 4-6x daily
            2. Apply warm compresses 2x daily
