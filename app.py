...
def analyze_fluorescein_staining(image):
    """Analyze fluorescein staining patterns"""
    try:
        # Convert to numpy array for processing
        img_array = np.array(image)
        
        # Simple analysis - look for green/yellow areas (fluorescein staining)
        # Convert to HSV color space approximation
        r, g, b = img_array[:,:,0], img_array[:,:,1], img_array[:,:,2]
        
        # Detect green/yellow areas (fluorescein staining)
        green_mask = (g > 150) & (r < 200) & (b < 200)
        yellow_mask = (r > 150) & (g > 150) & (b < 100)
        
        # Combine masks
        staining_mask = green_mask | yellow_mask
        ...
        result_array[staining_mask] = [255, 0, 0]  # Highlight in red
        
        result_image = Image.fromarray(result_array)
        
        # Determine staining grade
        if staining_percentage > 20:
            grade = "Severe (IV-V)"
            interpretation = "Significant corneal epithelial damage detected"
        elif staining_percentage > 10:
            grade = "Moderate (III)"
            interpretation = "Moderate epithelial disruption"
        elif staining_percentage > 5:
            grade = "Mild (II)"
            interpretation = "Mild epithelial changes"
        elif staining_percentage > 1:
            grade = "Trace (I)"
            interpretation = "Minimal epithelial staining"
        else:
            grade = "None (0)"
            interpretation = "No significant staining detected"
        ...
        st.error(f"Error in staining analysis: {e}")
        return None
    ...
    with tab2:
        ...
        with col1:
            ...
            staining_file = st.file_uploader("**Upload Fluorescein Staining Image**", 
                                           type=['jpg', 'jpeg', 'png'], 
                                           key="staining",
                                           help="Upload fluorescein staining image for corneal assessment")
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            if staining_file is not None and st.button("🔍 Analyze Fluorescein Staining", type="primary", use_container_width=True):
                with st.spinner("🔄 Analyzing fluorescein staining patterns..."):
                    ...
                    if staining_result:
                        ...
                        if staining_result['staining_percentage'] > 15:
                            st.markdown(f'''
                            <div class="warning-box">
                                <strong>⚠️ {staining_result["interpretation"]}</strong><br>
                                - Significant corneal epithelial damage<br>
                                - Consider immediate ophthalmological evaluation<br>
                            ...
                            <div class="recommendation-box">
                                ...
                                - Regular lubrication therapy<br>
...
<div style="text-align: center; color: #666; padding: 2rem;">
    ...
    <p><em>Final diagnosis and treatment decisions should be made by qualified ophthalmologists.</em></p>
